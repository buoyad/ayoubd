...
const readMDXFile = cache(async (filePath: string) => await fs.readFile(filePath, 'utf8'))
...
const getMDXContent = async (dir: string) => {
    const content = await Promise.all((await getMDXFiles(dir)).map(async (file) => {
        const filePath = path.join(dir, file)
        const { content, frontmatter } = await compileMDXFile(filePath)
        frontmatter.slug = encodeURIComponent((frontmatter.slug || frontmatter.title).replace(/\s/g, '-'))
