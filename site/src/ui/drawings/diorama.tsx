...
const noise = (x: number, y: number, z: number) =>
  _noise(x + noiseSeed, y + noiseSeed, z + noiseSeed)

const pickRandom = <T extends unknown>(arr: T[]) =>
  arr[Math.floor(Math.random() * arr.length)]
const grassColors = ['#136d15', '#117c13', '#138510', '#268b07', '#41980a']
...
const Landscape = () => {
  ...
  const genHeight = (ix: number, iz: number) => {
    ...
    const x = (ix / (maxX - minX)) * spatialScale,
      z = (iz / (maxZ - minZ)) * spatialScale
  ...
  while (
    lampPostPosition.distanceTo(maxHeightPosition) <= baseSize * 4 ||
    lampPostPosition.distanceTo(maxHeightPosition) >= baseSize * 6
  ) {
    lampPostPosition = pickRandom(positions)
  }
  const lampLightPosition = lampPostPosition.clone().add(new Vector3(0, 0.8, 0))
  ...
  return (
    <group>
      {boxes}
      <Model url={models.limeTree} scale={0.1} position={maxHeightPosition} />
      <Model
        url={models.lampPost}
        scale={0.3}
        position={lampPostPosition}
        onPointerDown={toggleLight}
      />
      <pointLight
        position={lampLightPosition}
        color={'yellow'}
        intensity={1}
        ref={lampRef}
      />
...
const Scene = ({ numStars = 100, container }: SceneProps) => {
  ...
  useFrame(({ camera }) => {
    const prev = new Spherical().setFromCartesianCoords(
      camera.position.x,
      camera.position.y,
      camera.position.z,
    )
  ...
  return (
    <>
      <hemisphereLight
        color="white"
        groundColor={pickRandom(grassColors)}
        intensity={1}
      />
      <spotLight
        position={[0, 0, 2]}
